  ...
  <div>
    ...
    <el-container class="cf-manage-pannel">
      <div class="cf-pannel-area">
        <svg class="cf-manage-close-btn" @click="close_manage_panel" viewBox="0 0 1024 1024"
...
import ManagePanelLogin from './ManagePanelLogin/ManagePanelLogin.vue'
import ManagePanelMain from './ManagePanelMain/ManagePanelMain.vue'
const LoginStore = useLoginStore();
const MainStore = useMainStore();
let { manage_panel_open } = storeToRefs(MainStore);
let { is_login_page_open } = storeToRefs(LoginStore);

// 关闭管理面板
function close_manage_panel() {
  MainStore.close_manage_panel()
}
// 登录成功
function login_success() {
  // 登录成功，不显示登录页面
  LoginStore.show_login_page()
}
// 退出登录
function logout() {
  // 展示登录页面
  LoginStore.close_login_page()
}
