    ...
    <el-tabs v-model="current_tab" type="card" class="cf-manage-tabs-area">
      <el-tab-pane label="配置管理" name="settings">
        <ManagePanelMain_settings v-if="current_settings !== null" :Config="Config" :current_settings="current_settings"
          :key="component_keys.settings" @refresh="refresh_component"></ManagePanelMain_settings>
      </el-tab-pane>
      ...
      <el-tab-pane label="数据库管理" name="switchsb">
        <ManagePanelMain_switchdb v-if="current_settings !== null" :Config="Config" :current_settings="current_settings"
          :key="component_keys.switchdb" @refresh="refresh_component"></ManagePanelMain_switchdb>
      </el-tab-pane>
...
// 切换当前组件
function change_current_component(component_name) {
  // 登录成功不显示登录页面
...
// 刷新内部的组件
function refresh_component(value) {
  // 重新获取当前配置
